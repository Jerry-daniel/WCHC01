/***********************************************************************************************************************
* File Name    : idt_rx_fail_ept_code_task.c
...
***********************************************************************************************************************/
unsigned char IDT_RX_Fail_Ept_Code_Process(void);
/***********************************************************************************************************************
* Function Name: IDT_RX_Fail_Ept_Code_Task
...
{
	if(IDT_WPC_State.Rx_Fail_Ept_Code_Task_Flag==TRUE)
	{
		IDT_WPC_State.Rx_Fail_Ept_Code_Task_Flag = FALSE;
		//---------------------------------------------//
		...
		//---------------------------------------------//
		IDT_WPC_TASK = IDT_WPC_RX_FAIL_EPT_CODE_TASK;
	}
	IDT_WPC_TASK = IDT_RX_Fail_Ept_Code_Process();
}
/***********************************************************************************************************************
* Function Name: IDT_RX_Fail_Ept_Code_Process
* Description  : This function is IDT_RX_Jiggle_Alarm_Task function.
* Arguments    : None
* Return Value : None
***********************************************************************************************************************/
unsigned char IDT_RX_Fail_Ept_Code_Process(void)
{
	if(ACC_Status.Active_On_Flag==TRUE)
	{
		if(Power_Sw.Active_Flag==TRUE)
		{
			...
			{
				if(Other_Alarm.Rx_Ept_Code_Flag==FALSE)
				{
				...
				else
				{
					if((Buzzer_State.Buzzer_Working_End_Flag==TRUE)&&(LED_Status.LED_Working_End_Flag==TRUE))
					{
						...
						Other_Alarm.Rx_Ept_Code_Flag = FALSE;
					}
					IDT_WPC_NEXT_TASK = IDT_WPC_RX_FAIL_EPT_CODE_TASK;
				}
			}
