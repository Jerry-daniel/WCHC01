/***********************************************************************************************************************
* File Name    : idt_receive_power_packet_timeout_task.c
...
***********************************************************************************************************************/
unsigned char IDT_Receive_Power_Packet_Timeout_Process(void);
/***********************************************************************************************************************
* Function Name: IDT_Receive_Power_Packet_Timeout_Task
...
{
	if(IDT_WPC_State.Received_Power_Packet_Timeout_Task_Flag==TRUE)
	{
		IDT_WPC_State.Received_Power_Packet_Timeout_Task_Flag = FALSE;
		//---------------------------------------------//
		...
		//}
		//---------------------------------------------//
		IDT_WPC_TASK = IDT_WPC_RECEIVE_POWER_PACKET_TIMEOUT_TASK;
	}
	IDT_WPC_TASK = IDT_Receive_Power_Packet_Timeout_Process();
}
/***********************************************************************************************************************
* Function Name: IDT_Receive_Power_Packet_Timeout_Process
* Description  : This function is IDT_RX_Jiggle_Alarm_Task function.
* Arguments    : None
* Return Value : None
***********************************************************************************************************************/
unsigned char IDT_Receive_Power_Packet_Timeout_Process(void)
{
	if(ACC_Status.Active_On_Flag==TRUE)
	{
		if(Power_Sw.Active_Flag==TRUE)
		{
			...
			{
				if(Other_Alarm.Receiver_Timeout_Flag==FALSE)
				{
				...
				else
				{
					if((Buzzer_State.Buzzer_Working_End_Flag==TRUE)&&(LED_Status.LED_Working_End_Flag==TRUE))
					{
						...
						Other_Alarm.Receiver_Timeout_Flag = FALSE;
					}
					IDT_WPC_NEXT_TASK = IDT_WPC_RECEIVE_POWER_PACKET_TIMEOUT_TASK;
				}
			}
