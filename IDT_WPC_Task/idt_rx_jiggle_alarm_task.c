/***********************************************************************************************************************
* File Name    : idt_rx_jiggle_alarm_task.c
...
***********************************************************************************************************************/
unsigned char IDT_RX_Jiggle_Alarm_Process(void);
/***********************************************************************************************************************
* Function Name: IDT_RX_Jiggle_Alarm_Task
...
{
	if(IDT_WPC_State.Rx_Jiggle_Alarm_Task_Flag==TRUE)
	{
		IDT_WPC_State.Rx_Jiggle_Alarm_Task_Flag = FALSE;
		//---------------------------------------------//
		...
		//---------------------------------------------//
		IDT_WPC_TASK = IDT_WPC_RX_JIGGLE_ALARM_TASK;
	}
	IDT_WPC_TASK = IDT_RX_Jiggle_Alarm_Process();
}
/***********************************************************************************************************************
* Function Name: IDT_RX_Jiggle_Alarm_Process
* Description  : This function is IDT_RX_Jiggle_Alarm_Process function.
* Arguments    : None
* Return Value : None
***********************************************************************************************************************/
unsigned char IDT_RX_Jiggle_Alarm_Process(void)
{
	if(ACC_Status.Active_On_Flag==TRUE)
	{
		if(Power_Sw.Active_Flag==TRUE)
		{
			...
			{
				if(Other_Alarm.Rx_Jiggle_Status_Flag==FALSE)
				{
				...
				else
				{
					if((Buzzer_State.Buzzer_Working_End_Flag==TRUE)&&(LED_Status.LED_Working_End_Flag==TRUE))
					{
						...
						Other_Alarm.Rx_Jiggle_Status_Flag = FALSE;
					}
					IDT_WPC_NEXT_TASK = IDT_WPC_RX_JIGGLE_ALARM_TASK;
				}
			}
