/***********************************************************************************************************************
* File Name    : idt_other_end_of_power_rx_alarm_task.c
...
***********************************************************************************************************************/
unsigned char IDT_Other_Eof_Pwr_Rx_Alarm_Process(void);
/***********************************************************************************************************************
...
{
	if(IDT_WPC_State.Other_Eof_Pwr_Rx_Alarm_Task_Flag==TRUE)
	{
		IDT_WPC_State.Other_Eof_Pwr_Rx_Alarm_Task_Flag = FALSE;
		//---------------------------------------------//
		...
		//---------------------------------------------//
		IDT_WPC_TASK = IDT_WPC_OTHER_EOF_PWR_RX_ALARM_TASK;
	}
	IDT_WPC_TASK = IDT_Other_Eof_Pwr_Rx_Alarm_Process();
}
/***********************************************************************************************************************
* Function Name: IDT_Other_Eof_Pwr_Rx_Alarm_Process
* Description  : This function is IDT_Other_Eof_Pwr_Rx_Alarm_Process function.
* Arguments    : None
* Return Value : None
***********************************************************************************************************************/
unsigned char IDT_Other_Eof_Pwr_Rx_Alarm_Process(void)
{
	if(ACC_Status.Active_On_Flag==TRUE)
	{
		if(Power_Sw.Active_Flag==TRUE)
		{
			...
			{
				if(Other_Alarm.Other_Eof_Pwr_Rx_Alarm_Status_Flag==FALSE)
				{
				...
				else
				{
					if((Buzzer_State.Buzzer_Working_End_Flag==TRUE)&&(LED_Status.LED_Working_End_Flag==TRUE))
					{
						...
						Other_Alarm.Other_Eof_Pwr_Rx_Alarm_Status_Flag = FALSE;
					}
					IDT_WPC_NEXT_TASK = IDT_WPC_OTHER_EOF_PWR_RX_ALARM_TASK;
				}
			}
